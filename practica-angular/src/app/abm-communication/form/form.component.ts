...
export class FormComponent implements OnInit {
  ...
  constructor(
    private api: ServiciosAPIService,
  ...
  eliminar(id: number) {
    var r = confirm(
      '¿Usted esta seguro que desea eliminar el empleado ' + id + ' ?'
    );
    if (r == true) {
      this.api.deleteEmployees(id).subscribe(
        () => this.ngOnInit(), //para que se muestren los cambios
        (error: HttpErrorResponse) => this.toastr.error('No se puede eliminar.', 'Error')
